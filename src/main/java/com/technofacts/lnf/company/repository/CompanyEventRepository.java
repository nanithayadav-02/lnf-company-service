...
public interface CompanyEventRepository extends JpaRepository<CompanyEvent, UUID> {
    ...
    @Query("SELECT e FROM CompanyEvent e WHERE e.eventType = :eventType AND EXTRACT(MONTH FROM e.dateAndTime) = :month")
    List<CompanyEvent> getEventsByMonth(@Param("eventType") EventType eventType, @Param("month") int month);

    @Query("SELECT e FROM CompanyEvent e WHERE e.eventType = :eventType AND EXTRACT(YEAR FROM e.dateAndTime) = :year")
